...
- [x] Task 5: 深度学习模型进阶 (Word2Vec + TextCNN/TextRNN)
    - **预训练嵌入**：使用 Gensim 构建 Word2Vec 模型，生成 100 维词向量，并导入 PyTorch 作为 Embedding 层的初始权重。
    - **架构对比实验 (基于 5 万条训练数据, Epoch=2)**：
        - **TextCNN**：耗时约 36 分钟，最终 Loss 降至 0.37。其并行的多尺度卷积核（2/3/4-gram）极为高效地捕捉了决定新闻类别的局部关键词特征。
        - **TextRNN (双向2层LSTM)**：耗时约 4 小时，最终 Loss 为 0.85。串行计算导致训练速度显著慢于 CNN；且在 200 词的长序列中，仅取最后时刻的隐藏状态容易产生信息瓶颈，导致在此类任务中收敛效果不及 CNN。
    - **研发结论**：针对长文本的新闻主题分类任务，局部特征（n-gram）的区分度高于长程时序逻辑。综合算力成本与收敛效果，**TextCNN 的工业落地性价比远高于传统 TextRNN**。
